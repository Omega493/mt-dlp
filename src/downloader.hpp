...
public:
  ...
  static FileInfo file_info(const std::string& url) {
    ...
    int32_t res_code{};
    const std::pair<std::string, CURLcode> curl_res_code{ "CURLINFO_RESPONSE_CODE", curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &res_code) };
    if (curl_res_code.second != CURLE_OK) {
      (void)curl_easy_cleanup(curl);
      throw std::runtime_error("Error while fetching curl's response code: "
        + curl_res_code.first + ": " + std::string{ curl_easy_strerror(curl_res_code.second) });
    }
  ...
  int download(const std::string& url, DownloadState& state, std::FILE* const fp, std::mutex& fp_mtx,
               const int64_t range_start = -1, const int64_t range_end = -1)
  {
    CURL* const curl{ curl_easy_init() };
    ...
    const std::vector<CurlOptRes> opts{
      ...
      { "CURLOPT_XFERINFOFUNCTION", curl_easy_setopt(curl, CURLOPT_XFERINFOFUNCTION, progress_callback) },
      { "CURLOPT_XFERINFODATA", curl_easy_setopt(curl, CURLOPT_XFERINFODATA, &ctx) },
      { "CURLOPT_RANGE", range_str.empty() ? CURLE_OK : curl_easy_setopt(curl, CURLOPT_RANGE, range_str.c_str()) }
    };
...
#ifdef _WIN32
      if (_fseeki64(ctx.fp, static_cast<__int64>(ctx.range_start_offset + ctx.bytes_flushed), SEEK_SET) != 0)
#else
      if (std::fseek(ctx.fp, static_cast<long>(ctx.range_start_offset + ctx.bytes_flushed), SEEK_SET) != 0)
#endif
...
private:
  ...
  struct Context {
    ...
    Context(DownloadState* const st, std::FILE* const f, std::mutex* const m, const int64_t start, const int64_t limit) :
      state{ st }, fp{ f }, fp_mtx{ m }, range_start_offset{ ((start < 0) ? 0 : start) },
      limit_bytes{ limit }, last_time{ std::chrono::steady_clock::now() }
    {
  ...
  static size_t header_callback(const char* const buffer, const size_t size, const size_t nitems, void* const userdata) {
  ...
  // 3/5 of the arguments are unused. They are still in the function definition because curl function ptr :staregePepe:
  static int progress_callback(void* const clientp, const curl_off_t, const curl_off_t dlnow, const curl_off_t, const curl_off_t) {
    Context* const ctx{ static_cast<Context*>(clientp) };

    const std::chrono::steady_clock::time_point now{ std::chrono::steady_clock::now() };
    const int64_t dur{ std::chrono::duration_cast<std::chrono::milliseconds>(now - ctx->last_time).count() };
  ...
  static size_t write_callback(const void* const contents, const size_t size, const size_t nmemb, void* const userp) {
...
#ifdef _WIN32
        if (_fseeki64(ctx->fp, static_cast<long long>(ctx->range_start_offset + ctx->bytes_flushed), SEEK_SET) != 0)
#else
        if (std::fseek(ctx->fp, static_cast<long>(ctx->range_start_offset + ctx->bytes_flushed), SEEK_SET) != 0)
#endif
          return 0; // Seek error
