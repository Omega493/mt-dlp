...
FileInfo Downloader::file_info(const std::string& url) {
  ...
  if (curl_easy_getinfo(curl, CURLINFO_CONTENT_LENGTH_DOWNLOAD_T, &size_dl) == CURLE_OK) {
    if (size_dl > 0) {
      info.size = static_cast<int64_t>(size_dl);
    }
  }
                         ...
                         const int64_t range_end)
{
  ...
  if (range_start >= 0) {
    ...
    if (range_end > 0) {
      range_str += std::to_string(range_end);
    }
  }
...

  {
    ...
    if (!state.should_retry) {  // Only mark done if we aren't retrying
      state.is_done = true;
      if (res != CURLE_OK) {
        state.err_msg = curl_easy_strerror(res);
      }
      else {
...
size_t Downloader::header_callback(const char* const buffer, const size_t size, const size_t nitems, void* const userdata) {
  ...
  if (selena::icontains(header_line, target)) {
    *supports_ranges = true;
  }
  return num_bytes;
}

int Downloader::progress_callback(void* const clientp, const curl_off_t, const curl_off_t dlnow, const curl_off_t, const curl_off_t) {
  ...
  const double curr_speed{ [&]() -> double {
    if (sec > 0) {
      return static_cast<double>(dlnow - ctx->last_byte_count) / sec;
    }
...
size_t Downloader::write_callback(const void* const contents, const size_t size, const size_t nmemb, void* const userp) {
  ...
  if (ctx->limit_bytes > 0) {
    if (ctx->bytes_processed >= ctx->limit_bytes) {
      return 0;  // Abort, we are done
    }
